...
- **No refactoring step after repetitive implementation**: Plans that create multiple similar components (5+ API endpoints, view handlers, or service methods) without including a refactoring task to extract shared patterns. Results in duplicated error handling, validation logic, or setup code across many files. After implementing similar features, plans should include a task to identify and extract common patterns into helpers, decorators, or base classes. Most relevant for REST APIs with similar endpoint structures, CRUD operations with repeated boilerplate, or service layers with identical error handling.
- **No shared utilities infrastructure**: Plans that create multiple similar modules (4+ page files, screens, services, or API handlers) without defining shared utility functions first. Results in common utilities (escapeHtml, formatTime, error formatting, validation helpers) being reimplemented in each module. Before building consuming modules, plans should create shared utility infrastructure and reference it in tasks. Examples: ES5 projects (Cookie.utils before page modules), React apps (lib/utils.ts before screens), Python apps (core/utils.py before app modules), Go apps (pkg/util before services). Most relevant when modules will clearly need similar helpers - don't create utilities speculatively, but do create them when 4+ modules will obviously share the same functions.
